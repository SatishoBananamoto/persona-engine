...
class RelationshipStore:
    ...
    Running scores are computed from the base + sum of deltas.
    """
    ...
    def trust(self) -> float:
        """Current trust level (0-1), clamped."""
        total = self._base_trust + sum(e.trust_delta for e in self._events)
        return max(0.0, min(1.0, total))

    @property
    def rapport(self) -> float:
        """Current rapport level (0-1), clamped."""
        total = self._base_rapport + sum(e.rapport_delta for e in self._events)
        return max(0.0, min(1.0, total))

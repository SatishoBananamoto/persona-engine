...
class TestConstructor:
    ...
    def test_mood_drift_rate_depends_on_neuroticism(self):
        ...
        # calibrated per psych literature: rate = 0.12 - neuroticism * 0.08
        # High N = mood lingers (slower drift), low N = faster recovery
    ...
    def test_stress_decay_rate(self, manager):
        # calibrated per psych literature: 0.06 + E*0.04 - N*0.03
        # manager fixture: E=0.5, N=0.5 -> 0.06 + 0.02 - 0.015 = 0.065
        assert manager.stress_decay_rate == pytest.approx(0.065)
...
class TestApplyMoodDrift:
    ...
    def test_drift_magnitude_uses_mood_drift_rate(self):
        ...
        # calibrated per psych literature:
        # rate = 0.12 - 0.5*0.08 = 0.08
        # baseline_valence = 0.1 + (0.5*0.15) - (0.5*0.2) = 0.075
        # baseline_arousal = 0.5
        sm.apply_mood_drift()
        # valence: 1.0 + (0.075 - 1.0)*0.08 = 1.0 - 0.074 = 0.926
...
class TestReduceStress:
    def test_normal_decay(self, manager):
        ...
        # calibrated per psych literature: stress_decay_rate = 0.065 for mid-range traits
        assert manager.state.stress == pytest.approx(0.435)
...
class TestCreateStateManager:
    ...
    def test_factory_uses_persona_traits(self):
        ...
        # calibrated per psych literature: mood_drift_rate = 0.12 - 0.8 * 0.08 = 0.056
